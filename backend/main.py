...
try:
    ...
    logging.warning("AI Assistant module not loaded: %s", _ai_err)
...
def create_high_risk_notifications(connection, project: sqlite3.Row | ProjectStat) -> None:
    ...
    for user_id, message in recipients:
        connection.execute(
            ...
            VALUES (?, ?, ?, 'HIGH_RISK')
            ON CONFLICT (user_id, project_id, alert_type) DO NOTHING
            """,
    ...
    with closing(get_connection()) as connection:
        if is_postgres_enabled():
            ...
            for name, role in users:
                connection.execute(
                    "INSERT INTO users (name, role, password_hash) VALUES (%s, %s, %s) ON CONFLICT (name, role) DO NOTHING",
